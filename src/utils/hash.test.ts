...
describe('hashValue', () => {
  ...
  test('should produce a bcrypt hash of input', async () => {
...
describe('compareHash', () => {
  ...
  test('compareHash should return false for invalid hash format', async () => {
    await expect(compareHash('someinput', 'not-a-valid-hash')).resolves.toBe(false);
  });

  test('should return true when raw and hash match', async () => {
    const hashed = await hashValue(rawString);
